import * as React from "react";

export const SAO = ({ size = 100 }: Props) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    id="Layer_1"
    x="0px"
    y="0px"
    width={size}
    height={size}
    viewBox="0 0 542.789 542.249"
    enableBackground="new 0 0 542.789 542.249"
    xmlSpace="preserve"
  >
    <polygon points="0,0 0,210.24 271.35,542.249 542.789,210.24 542.789,0 " />
    <polygon
      fill="#FFFFFF"
      points="3.6,3.6 3.6,208.979 271.35,536.49 539.191,208.979 539.191,3.6 "
    />
    <polygon points="314.64,198.629 314.64,439.739 511.829,198.629 " />
    <polygon
      fill="#ED1C24"
      points="30.96,198.629 228.24,439.739 228.24,198.629 "
    />
    <rect x={30.96} y={28.439} width={480.87} height={144.54} />
    <g>
      <path
        fill="#FFFFFF"
        d="M490.589,85.589v-15.12c0-11.07-9.089-20.16-20.16-20.16H419.49c-11.07,0-20.16,9.09-20.16,20.16v60.48   c0,11.07,9.09,20.16,20.16,20.16h50.939c11.071,0,20.16-9.09,20.16-20.16v-15.119h-20.16v10.079c0,2.79-2.249,5.04-5.04,5.04   H424.53c-2.79,0-5.04-2.25-5.04-5.04v-50.4c0-2.79,2.25-5.04,5.04-5.04h40.859c2.791,0,5.04,2.25,5.04,5.04v10.08H490.589z"
      />
      <path
        fill="#FFFFFF"
        d="M52.29,70.47v20.16c0,11.07,8.999,20.16,20.16,20.16h45.9c2.7,0,5.04,2.25,5.04,5.04v10.08   c0,2.79-2.34,5.04-5.04,5.04H52.29v20.16h71.1c11.069,0,20.16-9.09,20.16-20.16v-20.16c0-11.07-9.091-20.16-20.16-20.16h-45.9   c-2.79,0-5.04-2.25-5.04-5.04V75.51c0-2.79,2.25-5.04,5.04-5.04h66.06V50.31h-71.1C61.289,50.31,52.29,59.4,52.29,70.47"
      />
      <path
        fill="#FFFFFF"
        d="M239.129,50.31h-71.1v100.8h20.16v-40.32h50.94c11.071,0,20.07-9.09,20.07-20.16v-20.16   C259.199,59.399,250.2,50.31,239.129,50.31 M239.129,85.589c0,2.79-2.339,5.04-5.04,5.04h-45.9v-20.16h45.9   c2.701,0,5.04,2.34,5.04,5.04V85.589z"
      />
      <polygon
        fill="#FFFFFF"
        points="283.59,151.109 303.75,151.109 303.75,110.79 354.69,110.79 354.69,90.629 303.75,90.629    303.75,70.469 374.85,70.469 374.85,50.31 283.59,50.31  "
      />
    </g>
    <script
      xmlns=""
      type="text/javascript"
      src="chrome-extension://hejbmebodbijjdhflfknehhcgaklhano/../window/testing-library.js"
    />
  </svg>
);
